          ...
          {data.map((child) => (
            <Panel key={child.id}>
              ...
              <div className="mt-5 flex flex-wrap gap-2 text-sm font-semibold text-brand">
                <Link href={`/results/my?child=${child.id}`} className="inline-flex items-center gap-1 rounded-xl bg-brand-soft px-3 py-2 hover:underline">
                  <GraduationCap className="h-4 w-4" /> Results
                </Link>
                <Link href={`/attendance/mine?child=${child.id}`} className="inline-flex items-center gap-1 rounded-xl bg-brand-soft px-3 py-2 hover:underline">
                  <CalendarCheck className="h-4 w-4" /> Attendance
                </Link>
                <Link href="/finance" className="inline-flex items-center gap-1 rounded-xl bg-brand-soft px-3 py-2 hover:underline">
