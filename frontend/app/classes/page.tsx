              ...
              {classes.data.map((cls) => (
                <Panel key={cls.id} className="!p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="font-heading text-lg font-bold text-slate-900">{cls.name}</h3>
                      <p className="text-xs text-slate-500">{cls.studentCount} students</p>
                    </div>
                    <div className="flex gap-1">
                      <Button size="sm" variant="ghost" onClick={() => setArmFor({ classId: cls.id, name: "" })}><Plus className="mr-1 h-3.5 w-3.5" /> Arm</Button>
                  ...
                  {cls.arms.length === 0 ? (
                    <div className="mt-4 flex items-center justify-between rounded-2xl bg-slate-50 p-3 text-sm">
                      <span className="text-slate-600">Class teacher: <strong>{cls.classTeacherName ?? "Not assigned"}</strong></span>
                      <Button size="sm" variant="outline" onClick={() => setTeacherFor({ kind: "class", id: cls.id, label: cls.name, teacherId: cls.classTeacherId ?? "" })}><UserCheck className="mr-1 h-3.5 w-3.5" /> Assign</Button>
                    </div>
                  ) : (
                    <ul className="mt-4 space-y-2">
                      {cls.arms.map((arm) => (
                        <li key={arm.id} className="flex items-center justify-between rounded-2xl bg-slate-50 p-3 text-sm">
                          <div>
                            <span className="font-bold text-slate-800">Arm {arm.name}</span>
                            <span className="ml-2 text-xs text-slate-500">{arm.studentCount} students · Class teacher: {arm.classTeacherName ?? "none"}</span>
                          </div>
                          <div className="flex gap-1">
                ...
                <Panel key={subject.id} className="!p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="font-mono text-xs text-slate-500">{subject.code}</p>
                    </div>
                    <Badge variant={subject.isActive ? "success" : "secondary"}>{subject.isActive ? "Active" : "Inactive"}</Badge>
                  </div>
                  <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
                    <span>{subject.source === "GLOBAL" ? "From catalogue" : "Custom"}</span>
              ...
              <ul className="divide-y divide-slate-100">
                {assignments.data.map((a) => (
                  <li key={a.id} className="flex items-center justify-between py-3 text-sm">
                    <div>
                      <p className="font-bold text-slate-800">{a.teacherName}</p>
                      <p className="text-xs text-slate-500">{a.subjectName} · {a.classLabel}</p>
                    </div>
