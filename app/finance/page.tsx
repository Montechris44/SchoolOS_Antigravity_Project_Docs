...
export default function FinancePage() {
  ...
  const refreshData = () => {
    if (!school) return;
    setInvoices(db.getInvoices(securityCtx));
    setFeeStructures(db.getFeeStructures(securityCtx));
    setReceipts(db.getReceipts(securityCtx));
  ...
  const handleCreateInvoice = (e: React.FormEvent) => {
    ...
    db.createInvoice(securityCtx, {
      invoiceNumber: invoiceNum,
      studentId: student.id,
      studentName: `${student.firstName} ${student.lastName}`,
      guardianId: student.guardianId || "grd_parent",
      guardianName: student.guardianName || "Parent",
      guardianEmail: student.guardianEmail || "parent@example.com",
      guardianPhone: student.guardianPhone || "+2348000000000",
      termId: "trm_first_2026",
      termName: "First Term 2026/2027",
      items: [{ id: `it_${Date.now()}`, description: invoiceDescription, amount: Number(invoiceAmount) }],
