...
function PaymentsPageContent() {
  ...
  const refreshPayments = () => {
    if (!school) return;
    const p = db.getPayments(securityCtx);
    const inv = db.getInvoices(securityCtx);
    setPayments(p);
    setInvoices(inv);
  ...
  const handlePaystackPayment = async (isDuplicateTest: boolean = false) => {
    ...
    try {
      // Dispatch verified webhook event to server-side endpoint
      const webhookPayload = {
        event: "charge.success",
        data: {
          reference: testReference,
          amount: Math.round(customAmount * 100), // Kobo minor unit
          status: "success",
          paid_at: new Date().toISOString(),
          customer: {
            email: invoice.guardianEmail || user?.email || "payer@example.com",
          },
          metadata: {
            schoolId: school?.id,
      ...
      setPaymentLog({
        type: "warning",
        message: `Payment simulation note: ${err.message}`,
      });
    } finally {
      setIsProcessing(false);
    }
