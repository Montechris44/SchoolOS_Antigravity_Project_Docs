...
import { Student, SchoolClass, Staff, Subject, Guardian } from "@/types";
...
export default function PeopleAndClassesPage() {
  ...
  const refreshData = () => {
    if (!school) return;
    setStudents(db.getStudents(securityCtx));
    setClasses(db.getClasses(securityCtx));
    setStaff(db.getStaff(securityCtx));
    setSubjects(db.getSubjects(securityCtx));
  };
  ...
  const handleEnrollStudent = (e: React.FormEvent) => {
    ...
    // Check admission number uniqueness invariant (as per data-model.md)
    const existing = students.find(
      (s) => s.admissionNumber.trim().toUpperCase() === formData.admissionNumber.trim().toUpperCase()
    );
    if (existing) {
      setModalErrorMsg(`Admission number '${formData.admissionNumber}' is already registered in this school.`);
      return;
    }

    const targetClass = classes.find((c) => c.id === formData.classId) || classes[0];

    db.addStudent(securityCtx, {
      ...
      guardianName: `${formData.guardianFirstName} ${formData.guardianLastName}`.trim() || undefined,
